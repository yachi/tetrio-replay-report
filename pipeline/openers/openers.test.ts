...
test('no round comes within four cells of a catalogued C-Spin', () => {
  ...
  // 2026-09-03 takes it 907 -> 981, and the zero is unmoved at nine sessions.
  // 2026-09-11 takes it 1095 -> 1180, and the zero is unmoved at eleven.
  ...
  // answers it: yachi's replay-2026-08-14-2.ttrm round 3 is an EXACT DT Cannon first bag, as drawn and
  // mirrored — a different opener, found by the same instrument. It is also the corpus's only
  // Double-before-Triple opener (see DT_ORDER_IN_OPENER below), and the two metrics reach that round
  // from different inputs: this one from board occupancy at the end of bag 1, that one from spin lock
  ...
  // THREE now. 2026-09-17 adds replay-2026-09-17-07.ttrm round 3, and it is again the SAME round
  // the ordering metric flags Double-before-Triple. That session holds TWO Double-first rounds and
  // this metric reaches only one of them, which is the useful part: the corroboration is 3 of the
  // corpus's 5 Double-first openers, and the session supplying both a corroborated and an
  // uncorroborated one — same player, same night — is what rules out the session and the player as
  // the thing that decides which.
  //
  // STILL THREE AT THIRTEEN SESSIONS, and that is now the interesting part rather than a
  // continuation. 2026-09-19 contributes TEN Double-first openers (DT_ORDER_IN_OPENER below) and
  // this metric reaches NONE of them: the session has zero exact DT Cannon first bags and both
...
/** The ratio the 「never rivals」 sentence rests on, as a CLAIM rather than as a measurement.
 *
 *  This was `dt_order < cspin_order / 10` asserted per player per session, and 2026-09-17 broke it
 *  without breaking anything the section says: yachi ran 2 DT orders against 10 C-Spin orders, a
 *  1-in-5 ratio, because only 12 of his 47 rounds held both spins at all. A per-player-session
 *  ratio over a 12-round denominator is a small-n statement, and relaxing the constant to /5 would
 *  have been fitting the bound to the night — the next session can always produce a smaller
 *  denominator.
 *
 *  What is NOT lost by pooling: `dt_order` is still pinned EXACTLY per player per session, against
 *  the named exception list two lines below, so a new Double-first round still fails there and has
 *  to be investigated. What pooling buys is that the surviving inequality is the corpus claim the
 *  section actually prints, measured where its denominator is 857 rounds rather than 12.
 *
 *  The bound is the claim (a twentieth), deliberately NOT the measurement (1 in 171 at twelve
 *  sessions) — a guard set to today's number is a copy of today's number. */
const DT_RIVALS_CSPIN_AT = 20;

test('the ordering metric separates the two openers, in every session, both players', () => {
  const pooled = { window: { dt: 0, cspin: 0 }, full: { dt: 0, cspin: 0 } };
  for (const s of SESSIONS) {
    const data = JSON.parse(readFileSync(`${sessionDir(s)}/sim/opener-facts.json`, 'utf8'));
    for (const [scope, players] of [['window', data.ordering.players],
                                    ['full', data.ordering_full_round.players]] as const) {
      for (const p of players) {
        ...
        // DT Cannon is a Double then a Triple; the C-Spin is a Triple then a Double. The finding
        // this section carries is that every round holding both spins runs the C-Spin order EXCEPT
        // the ones named in DT_ORDER_IN_OPENER — five rounds in twelve sessions — pinned so a
        // change to the simulator has to face it. Subtracting a named exception rather than
        // relaxing to an inequality keeps the assertion exact: a sixth Double-first round still
        // fails here.
        ...
        expect([s, p.user, scope, p.dt_order]).toEqual([s, p.user, scope, known]);
        pooled[scope].dt += p.dt_order;
        pooled[scope].cspin += p.cspin_order;
      }
    }
  }
  // ... and pooled over the corpus it never RIVALS the C-Spin order, which is the shape of the
  // result the section prints. Both scopes, because the full-round metric is the control on the
  // windowed one and a claim that held for only one of them would be the more interesting fact.
  for (const scope of ['window', 'full'] as const) {
    expect([scope, pooled[scope].dt > 0]).toEqual([scope, true]);   // not vacuous: the DT order exists
    expect([scope, pooled[scope].dt * DT_RIVALS_CSPIN_AT < pooled[scope].cspin])
      .toEqual([scope, true]);
  }
});
...
/** The ONE session in which pinglamb's boards exactly match a TKI-3 field, named rather than
 *  absorbed — the `DT_ORDER_IN_OPENER` / `CAVE_IN_OPENER_EXCEPTIONS` precedent a third time.
 *
 *  READ THE TWO CLAIMS APART, because only the stronger one broke:
 *
 *    the ORDINAL claim  — "yachi opens TKI-3 more than pinglamb does". UNBROKEN, nine sessions:
 *                         2026-09-03 is 10 against 1. This is the claim the section publishes.
 *    the ABSOLUTE claim — "pinglamb never plays TKI-3 at all". Held 0 for eight sessions and
 *                         2026-09-03 breaks it with a single board.
 *
 *  It is a real detection, not a near-miss absorbed by the threshold: the match is EXACT
 *  (`min_cells` 0, so the board equals the drawn field cell for cell) and it is corroborated by
 *  the outcome column — `matched_and_delivered` is 1 of 1, i.e. that round went on to do what
 *  TKI-3 is for. A shape with no outcome behind it would be a coincidence of stacking; this one
 *  has one.
 *
 *  Pinned as a LIST rather than by relaxing the assertion to `<= 1`: a bound would be satisfied by
 *  any single pinglamb TKI-3 board in any session, while this names the session and the count, so
 *  a second one — or this one moving — fails and has to be investigated the same way. */
//  2026-09-19 adds THREE at once — all pinglamb's, all delivered — which takes the corpus from
//  two off-repertoire TKI-3 boards to five and is the point at which 「a third still has to be
//  investigated」 stops being the right instruction. Three in one night is not three more
//  exceptions; it is a rate (3 of his 146 scored rounds) where the first two were events. The
//  ORDINAL claim is untouched and is what the test below actually asserts: 23 against 3 this
...
test('the repertoire split reproduces in every session', () => {
  ...
  // Mountainous Stacking far more than pinglamb and plays TKI-3 far more than pinglamb does.
  // Nine independent sessions, so this is a regression rather than an observation.
  //
  // WHAT IS ASSERTED HERE IS THE ORDERING, NOT THE MAGNITUDE, and that gap is a live defect
  // elsewhere: CLAUDE.md publishes per-session RANGES beside these claims (pinglamb Honey Cup
  // 17-25, yachi MS1 11-25, yachi TKI-3 5-8) which nothing in this repo re-derives. Two of them
  // were already false when 2026-08-14 landed and stayed published for five days; the true
  // seven-session ranges are 17-30, 11-26 and 5-15. Asserting a range here is deliberately NOT
  // done — the counts move every session and a pinned band would be noise — so if that prose is
  // to stay, it needs a gate that recomputes it, not a corrected number.
  for (const s of SESSIONS) {
    const data = JSON.parse(readFileSync(`${sessionDir(s)}/sim/opener-facts.json`, 'utf8'));
    ...
    // THE ORDINAL CLAIM, which is what nine sessions actually reproduce. It used to read
    // `.toBe(0)` — the absolute — and 2026-09-03 broke it with one exact, delivered board. The
    // ordering is asserted here and the single exception is named above, so the count coming off
    // zero was a failure to read rather than a number to absorb.
    expect([s, of('tki_3', 'pinglamb').exact]).toEqual([s, TKI3_OFF_REPERTOIRE[s] ?? 0]);
    expect(of('tki_3', 'yachi').exact).toBeGreaterThan(of('tki_3', 'pinglamb').exact);
  }
});
...
const PERFECT_CLEARS: Record<string, Record<string, number>> = {
  ...
  '2026-09-03': { yachi: 4, pinglamb: 6 },
  '2026-09-10': { yachi: 5, pinglamb: 9 },
  '2026-09-11': { yachi: 8, pinglamb: 14 },
...
const IN_PCO_WINDOW: Record<string, number> = {
  ...
  '2026-08-14': 0, '2026-08-19': 0, '2026-08-25': 0, '2026-09-03': 0,
  // the fourth in the corpus, and the first since 2026-08-01: yachi lands one on piece 10,
  // exactly harddrop's deadline. 20 more sessions' worth of clears have arrived since the
  // numerator last moved, so the reading ('these are mid-game PCs') is reinforced, not upset.
  '2026-09-10': 1,
  // back to 0, on a session holding 22 perfect clears — the corpus's second-highest. The numerator
  // moving on 09-10 and not here is what the RATE reading survives on: 4 of 160 is not a streak of
  // zeroes and never was one.
  '2026-09-11': 0,
  // the fifth in the corpus, and yachi again lands it on piece 10 — exactly harddrop's deadline,
  // the same boundary as 09-10's. Two moves in three sessions is what retires the last of the
  // streak reading: the numerator is not stuck, it is rare, and the RATE is what has held.
  '2026-09-17': 1,
  // 2026-09-19 adds 32 perfect clears and NONE inside the window, taking the corpus to 5 of 199.
  // The largest single contribution the denominator has ever had, and the numerator does not
  // move — which is the rate reading doing what it is for. Do not restate it as a streak: the
  // previous two sessions each moved the numerator, and a zero after two ones is not a return
  // to anything.
  '2026-09-18': 0,
  '2026-09-19': 0,
};
...
const DONATION: Record<string, { donations: number; natural: number; b2b: number; clears: number }> = {
  ...
  '2026-09-03': { donations: 13, natural:  9, b2b: 0, clears: 518 },
  '2026-09-10': { donations: 15, natural: 12, b2b: 1, clears: 612 },
  '2026-09-11': { donations: 16, natural:  9, b2b: 3, clears: 603 },
...
const CAVE: Record<string, { width_ge_3: number; min_depth_ge_2: number; triple_control: number }> = {
  ...
  '2026-09-03': { width_ge_3:  6, min_depth_ge_2: 0, triple_control:  5 },
  '2026-09-10': { width_ge_3:  3, min_depth_ge_2: 0, triple_control:  5 },
  '2026-09-11': { width_ge_3:  6, min_depth_ge_2: 0, triple_control:  7 },
...
const TSPIN_ANCHOR: Record<string, { rounds: number; replay: number; scored: number }> = {
  ...
  '2026-09-03': { rounds:  92, replay: 548, scored: 518 },
  '2026-09-10': { rounds: 130, replay: 683, scored: 612 },
  '2026-09-11': { rounds: 102, replay: 653, scored: 603 },
...
const DUAL: Record<string, { comparable: number; scored: number; sameBoard: number;
                             cave: [number, number] | null; don: [number, number] | null }> = {
  ...
  '2026-09-03': { comparable: 216, scored: 518, sameBoard: 112, cave: [1, 1], don: [0, 7] },
  '2026-09-10': { comparable: 278, scored: 612, sameBoard: 168, cave: [2, 2], don: [0, 2] },
  '2026-09-11': { comparable: 245, scored: 603, sameBoard: 149, cave: [1, 1], don: [1, 6] },
  // the cave's oracle_positives is 0 here — the first session where the comparison reaches no cave
  // at all, so this session contributes nothing to the cave's positives row rather than a zero
  // agreement. `agreement_on_positives` is null for it, which is the honest spelling of "no
  // evidence" and is not the same as "the engines disagreed".
 ...
 *
 *  `sum(self_built_well) === 0` held for nine sessions and read as a property of these players.
 *  2026-09-10 breaks it with two, both yachi's, and both survive the checks that would make a
 *  detection cheap: `replay-2026-09-10-01.ttrm` round 6 plugs a 4-cell cavity at lock 40 of a
 *  prefix verified to 64, and `replay-2026-09-10-07.ttrm` round 7 a 5-cell cavity at lock 75 of
 *  one verified to 82 — so neither is a prefix-edge effect, and both are `natural` plugs, i.e.
 *  the plugging lock cleared nothing. Both are mid-game (lock > WINDOW_PIECES).
 *
 *  A count, not a bound: `<= 2` would be satisfied by any two self-built wells anywhere in the
 *  corpus, and this is satisfied only by a session whose total is exactly its entry. A third has
 *  to be investigated rather than absorbed. This is the third 「never happens」 in this file to
 *  turn out to be 「has not happened yet」, after DT_ORDER_IN_OPENER and CAVE_IN_OPENER_EXCEPTIONS.
 *
 *  THE THIRD ARRIVED, and it arrived the way the list is built to make one arrive:
 *  `replay-2026-09-11-02.ttrm` round 4, yachi again, a 4-cell cavity at lock 135 of a prefix
 *  verified to 219, `natural` plug, mid-game. Same player, same shape, same class as 09-10's two.
 *  What that buys is smaller than it looks and is worth stating plainly: three events across two
 *  adjacent sessions, all one player's, is consistent with a habit and equally consistent with two
 *  nights — the column separates whose WELL it was, never whether the player meant to build it. So
 *  this stays a named count and not a rate, and the fourth is to be investigated rather than
 *  absorbed. Note also what has NOT changed: pinglamb is 0 across all eleven sessions. */
const SELF_BUILT_WELLS: Record<string, number> = { '2026-09-10': 2, '2026-09-11': 1 };
...
test('the donation counts are pinned per session, and the licensing check is clean', () => {
  for (const s of SESSIONS) {
    ...
    // The provenance split, as a NAMED exception list rather than the `=== 0` absolute this was
    // for nine sessions — the `DT_ORDER_IN_OPENER` precedent a fourth time. The caveat the section
    // may not drop is unchanged and does not rest on this column: the oracle keeps the engine's
    // seeded-RNG hole columns, so WHICH column was donated into is never established either way.
    // What the column does say is whose well the shape sat on, and 2026-09-10 is the first session
    // where the answer is not "garbage, every time".
    expect([s, sum(d.players.map((p: any) => p.self_built_well))])
      .toEqual([s, SELF_BUILT_WELLS[s] ?? 0]);
    expect(sum(d.players.map((p: any) => p.garbage_derived_well)))
      .toBe(want.donations - (SELF_BUILT_WELLS[s] ?? 0));
...
test('the donation rate is nowhere near the naive reading, in every session', () => {
  ...
  // thresholds with the re-opening clause deleted it fires on 28.9-36.8% (re-measured over seven
  // sessions 2026-08-19; 08-19's 36.13% sits just inside 08-14's 36.84% ceiling).
  ...
  // a line-clear counter. Measured: 2.1-3.3% across the seven sessions.
  //
  // THE OTHER PUBLISHED BAND IS NOT MEASURED HERE and is the fragile one: the `cavity >= 1`
  // composite runs 74.60-77.04%, whose one-decimal ceiling (77.0%) survives only because 08-14's
  // 77.04% ties 08-09's 76.99% at that precision. 08-19 came in at 76.51% and did not move it, but
  // a future session anywhere in 77.05-77.94% still would — and no gate in this repo re-derives
  // that band, so it has to be re-measured by hand each session.
  for (const s of SESSIONS) {
    const d = facts(s).donation;
...
test('the cave metric\'s own controls have teeth: >=3 wide is not a cave count', () => {
  ...
  //   under Doubles, off a much smaller base. (Seven sessions: 58 under Triples to 49 under
  //   Doubles.)
  ...
  //   BY DEPTH — nearly every hit is one row deep. 48 dimples to 1 cave over the corpus, so the
...
const MID_GAME_ORDER: Record<string, { rounds_with_both: number; cspin: number; dt: number }> = {
  ...
  '2026-09-03': { rounds_with_both: 2, cspin: 2, dt: 1 },
  '2026-09-10': { rounds_with_both: 3, cspin: 3, dt: 1 },
  '2026-09-11': { rounds_with_both: 5, cspin: 3, dt: 3 },
...
const WINDOW_SPLIT: Record<string, { donation: [number, number]; cave: [number, number] }> = {
  ...
  '2026-08-14': { donation: [6, 15], cave: [0, 9] },
  // NOT `cave: [0, ...]` — 2026-08-19 is the first session with a >=3-wide hit INSIDE the opener
  ...
  '2026-09-03': { donation: [5, 8], cave: [1, 5] },
  '2026-09-10': { donation: [4, 11], cave: [0, 3] },
  '2026-09-11': { donation: [6, 10], cave: [0, 6] },
 ...
 *  ordering paragraph had a branch for "completely no overlap". 2026-08-14 broke it: yachi opened
 *  replay-2026-08-14-2.ttrm round 3 (m3r4) with a T-spin Double on lock 13 and a T-spin Triple on
 *  lock 18 — both inside the 21-lock window, and that round's verified prefix runs to lock 208, so
 *  it is not the short-prefix artefact the window control exists to rule out.
 *
 *  It is pinned as an EXCEPTION LIST rather than by relaxing the assertion to `<= 1`. A bound would
 *  be satisfied by any single Double-first round anywhere; this names the one, so a second one — or
...
const DT_ORDER_IN_OPENER: Record<string, Record<string, number>> = {
  ...
  //  the other metric.
  '2026-08-25': { yachi: 1 },
  //  THE THIRD, and it breaks the pattern the first two established rather than extending it.
  //  pinglamb opened replay-2026-09-10-07.ttrm round 2 (m7r3) Double-before-Triple inside the
  //  window; the prefix verifies to lock 67, so it is not a short-prefix artefact either. Two
  //  things are new. It is the first that is NOT yachi's, and — the one that matters — it is the
  //  first with NO first-bag corroboration: 2026-09-10 has zero exact matches of any opener, and
  //  pinglamb's nearest DT Cannon first bag in the session is 8 cells away. So the pairing is 2
  //  of 3, not 3 of 3. 「Both positives have two independent metrics naming the same round」 was
  //  a statement about two rounds and must not be carried forward as a property of the metric;
  //  what survives is that the ordering metric finds Double-first openers the first-bag metric
  //  sometimes cannot see, which is what two metrics scored on different inputs are FOR.
  '2026-09-10': { pinglamb: 1 },
  //  THE FOURTH AND FIFTH, both yachi's, both in this one session — and between them they answer
  //  the question the third one left open, which was which of the two patterns a fourth would
  //  join. The answer is ONE EACH, so the split is not a property of the session or the player:
  //
  //    replay-2026-09-17-02.ttrm round 6 (m2r7) — prefix verified to lock 28, past the 21-lock
  //      window, so not a short-prefix artefact. NO first-bag corroboration.
  //    replay-2026-09-17-07.ttrm round 3 (m7r4) — prefix verified to lock 103. The first-bag
  //      metric independently reaches this exact round as an exact DT Cannon match, as
  //      2026-08-14's and 2026-08-25's did.
  //
  //  Corpus-wide the corroboration is now 3 of 5. What makes this session the informative one is
  //  that both patterns occur in it, for the same player, on the same night — so whatever decides
  //  whether the first-bag metric sees a Double-first opener, it is not the session and not who is
  //  playing. It is the round.
  '2026-09-17': { yachi: 2 },
  //  THE SIXTH THROUGH FIFTEENTH — TEN OF THEM, IN ONE SESSION, AND THIS IS WHERE THE
  //  「investigate each one」 DISCIPLINE STOPS SCALING. Eight of yachi's and two of pinglamb's,
  //  spread over seven different files. They are not a prefix artefact: `ordering_full_round`
  //  drops the verification requirement entirely and finds the same ten, so nothing here is the
  //  verified prefix ending early. Every one carries exactly one Double and one Triple in its
  //  window, which is the same shape as all five earlier cases.
  //
  ...
  //
  //  NO first-bag corroboration for any of the ten: 2026-09-19 has zero exact DT Cannon first
  //  bags and both players' nearest is 6 cells away. **So the corroboration is 3 of 15, not 3 of
  //  5**, and the ratio moved because the denominator tripled rather than because the metric got
  //  worse. Do not carry 「3 of 5」 forward — it was a statement about five rounds.
  //
  //  This stays a named table because the assertion's shape is what has teeth (a count the data
  //  does not support fails from either side), but the per-round investigation that justified
  //  entries one through five is NOT claimed for these ten. Saying so is the point: an exception
  //  list whose entries were each looked at, and one whose entries were counted, are different
  //  evidence, and the file should not let them look the same.
  '2026-09-19': { yachi: 8, pinglamb: 2 },
  //  THE SIXTEENTH THROUGH NINETEENTH, on the night BEFORE 09-19 — the first entries this table
  //  has ever gained anywhere but at the end, because 2026-09-18 is the first session added out
  //  of date order. One of yachi's and three of pinglamb's, in four different files:
  //
  //    replay-2026-09-18-02.ttrm round 2, pinglamb — prefix verified to lock 86. Its window
  //      carries ONE Double and TWO Triples, so it reads Triple-Double-Triple and registers BOTH
  //      orders, exactly as 2026-09-10's m7r3 did. It is therefore in this table and NOT in
  //      DT_ONLY_IN_OPENER below — the second such round the corpus holds, and the reason the
  //      comment there saying 09-10 is the only session where the two lists differ is now wrong.
  //    replay-2026-09-18-06.ttrm round 0, yachi — prefix verified to lock 81.
  //    replay-2026-09-18-07.ttrm round 6, pinglamb — prefix verified to lock 30.
  //    replay-2026-09-18-13.ttrm round 0, pinglamb — prefix verified to lock 135.
  //
  //  Every prefix runs well past the 21-lock window, so none is a short-prefix artefact. NO
  //  first-bag corroboration for any of the four: this session has zero exact DT Cannon first
  //  bags. Corpus-wide the corroboration is therefore 3 of 19.
  //
  //  What this session does to the RATE sentence below is the part worth reading. Per round
  //  holding both spins the DT order now runs 0 · 0 · 0 · 0 · 0 · 0.99 · 0 · 1.11 · 0 · 1.22 ·
  //  0 · 4.00 · 3.15 · 6.06 %, so the last THREE sessions hold 16 of the corpus's 19 on 342 of
  //  its 1149 rounds — 4.68% against 0.37% for the first eleven. Three sessions is still not a
  //  trend by this repo's own rule and no mechanism is proposed; what it does retire is any
 ...
 *
 *  NOT the same quantity as `DT_ORDER_IN_OPENER`, and 2026-09-10 is what proved it. These three
 *  assertions used to read `cspin_order === rounds_with_both - DT_ORDER_IN_OPENER[...]`, which
 *  silently assumed a round showing the DT order cannot also show the C-Spin order. That holds
 *  when the window carries exactly one Double and one Triple, which is every case the corpus had
 *  through nine sessions. pinglamb's replay-2026-09-10-07.ttrm round 2 carries ONE Double and TWO
 *  Triples, so the window reads Triple-Double-Triple and registers BOTH orders: `cspin_order` 1,
 *  `dt_order` 1, in one round. The old form then expected 45 against the artefact's 46.
 *
 *  The subtraction is over rounds that showed no Triple before a Double, which is what
 *  `rounds_with_both - cspin_order` means, so this list is the honest operand. It stays a named
 *  list for the same reason the one above is: a value here that the data does not support fails,
 *  and so does a missing one. */
const DT_ONLY_IN_OPENER: Record<string, Record<string, number>> = {
  '2026-08-14': { yachi: 1 },
  '2026-08-25': { yachi: 1 },
  // 2026-09-10 is deliberately absent: its DT round showed the C-Spin order too, so it subtracts
  // nothing. An entry of 1 here would fail, which is the point of listing rather than bounding.
  //
  // 2026-09-17's two ARE both DT-only — each window carries exactly one Double and one Triple — so
  // the two lists agree at 2 for this session and disagree at 09-10. That the same session can be
  // present in both with the same number and present in one only is what keeps them separate
  // quantities rather than one with two names.
  '2026-09-17': { yachi: 2 },
  // 2026-09-19's ten are all DT-only — every window carries exactly one Double and one Triple —
  // so the two lists agree again at ten.
  '2026-09-19': { yachi: 8, pinglamb: 2 },
  // 2026-09-18 is the SECOND session where the two lists differ, and 09-10 is no longer the only
  // one. Three of its four DT rounds are DT-only; pinglamb's replay-2026-09-18-02.ttrm round 2
...
test('the mid-game denominator is far too small to be published as a rate', () => {
  // Twelve rounds against 455. The 全消 section refuses a percentage over 3-12 rounds for the same
...
/** The >=3-WIDE HITS that fall inside the opener window, named rather than absorbed.
 *
 *  READ THE TWO CLAIMS APART — they are not the same claim and only the weaker one broke.
 *
 *    the SHAPE claim  — "no >=3-wide hit falls inside the opener window". Held for six sessions
 *                       (0 of 39) and 2026-08-19 BREAKS it, with the two entries below.
 *    the CAVE claim   — "no genuine cave falls inside the opener window". UNBROKEN. Both entries
 *                       are `minDepth 1`, i.e. dimples, and 2026-08-19's `min_depth_ge_2` is 0, so
 *                       neither is a cave by the metric's own depth column. The corpus still holds
 *                       exactly ONE genuine cave in 900 player-rounds and it is not in an opener.
 *
 *  So these are NOT counter-evidence to harddrop's filing of the technique under `Mid-game T-Spin
 *  setups`. What they cost is the right to state that filing as an ABSOLUTE over the raw width
 *  column — which is the same trap `width_ge_3` sets everywhere else in this file, arriving here
 *  from a new direction. A reader must not come away thinking a cave appeared in an opener.
 *
 *  `verifiedTo` is carried because it is what rules out the artefact the window control exists to
 *  catch: both rounds are verified far past the spin (62 and 64 against locks 20 and 17), so
 *  neither is a short-prefix accident.
 *
 *  UNEXPLAINED, and recorded as such: both hits are in the same replay file, in consecutive rounds,
 *  one for each player. No mechanism is proposed here — two events are not a pattern, and inventing
 *  a story for them is how a coincidence becomes a finding.
 *
 *  Pinned as a LIST rather than by relaxing the assertion to a bound, following DT_ORDER_IN_OPENER:
 *  a bound is satisfied by any two in-opener hits anywhere, while this names these two, so a THIRD
 *  one — or either of these moving — fails and has to be investigated the same way. */
const CAVE_IN_OPENER_EXCEPTIONS = [
  { session: '2026-08-19', file: 'replay-2026-08-19-9.ttrm', round: 4, user: 'pinglamb',
  ...
  //  2026-09-03 adds a fifth, and it ANSWERS the question the fourth left open rather than
  //  compounding it. The note above says a fifth exception arriving at its boundary would, with
  //  08-25's, be a reason to suspect the prefix edge. This one is at lock 18 with the prefix
  //  verified to 45 — two and a half times past the spin, so it is ruled out as an edge effect
  //  the way the first three are. 08-25's boundary hit therefore stays a single unexplained
  //  instance and does NOT become a pattern; the edge suspicion is not raised.
  //
  //  `minDepth` is 1 again and 09-03's `min_depth_ge_2` is 0, so the CAVE claim is untouched for
  //  the third session running. Five raw-width exceptions, still exactly one genuine cave in the
...
test('the STMB cave is a mid-game shape, and that is measured rather than cited', () => {
  ...
  for (const s of SESSIONS) {
    for (const p of facts(s).stmb_cave.players) {
      const known = CAVE_IN_OPENER_EXCEPTIONS
        .filter(e => e.session === s && e.user === p.user).length;
      expect([s, p.user, p.in_opener]).toEqual([s, p.user, known]);
      inOpener += p.in_opener;
    }
    // THE CAVE CLAIM, enforced rather than commented: a session may only carry an in-opener hit
    // while it has no genuine cave at all. That is what licenses calling these dimples — if a
    // session ever has both, this fails and the two claims can no longer be separated by session.
    if (CAVE_IN_OPENER_EXCEPTIONS.some(e => e.session === s))
  ...
  // arithmetic, and broke on the same day that stopped being true. The invariant it MEANT to assert
  // is that the window partitions the hits, which is this: the two columns sum to the total.
  expect(out + inOpener).toBe(sum(SESSIONS.map(s => CAVE[s]!.width_ge_3)));
});
