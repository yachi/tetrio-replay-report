...
test('no round comes within four cells of a catalogued C-Spin', () => {
  ...
  // -> 471 with confirm-timed garbage, -> 477 with triangle's DAS/ARR port, -> 490 with exact-subframe
  // event processing). The finding is the line below — `d <= 4` is still empty — not the population.
  expect(clean.length).toBe(490);
  ...
  // with the DAS/ARR port, -> 366 with exact-subframe event processing; still the bulk of 490 clean)
  expect(clean.filter(r => r.sbTriple).length).toBe(366);
}, 300_000);
