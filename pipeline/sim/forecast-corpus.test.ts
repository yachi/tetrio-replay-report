...
const PINNED_FORECASTS: Record<string, string[]> = {
  ...
  '2026-09-11': [],
  // 0 again, but for 08-14's reason rather than 09-11's: there IS a mechanism event and it is
  // rejected downstream. The two adjacent zeroes therefore have different causes, which is why
...
const PINNED_MECHANISM_ONLY: Record<string, string[]> = {
  ...
  '2026-09-11': [],
  // One event, rejected downstream, so 2026-09-11 stays the only empty entry. It sits in
  // replay-2026-09-17-07.ttrm r3 — which is also the round the ordering metric and the first-bag
