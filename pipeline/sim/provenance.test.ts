 ...
 *   1. the planted mutants — a gate whose selftest has stopped firing gates nothing;
 *   2. the corpus, over BOTH published board sources;
...
import { provenanceViolations, roofAdmissibility, selftest, sessionDirs, sweep } from './check_provenance.ts';

const ROOT = `${import.meta.dir}/../..`;
const DIRS = sessionDirs(ROOT);
const SOURCES = ['oracle-source.ts (published)', 'sim.ts hand-port (published)'];

test('the gate still fires: every planted mutant is caught', () => {
  const lines: string[] = [];
  expect(selftest(s => lines.push(s))).toBe(0);
  // A selftest that silently stopped planting anything would also return 0.
  expect(lines.filter(l => l.startsWith('  ok ')).length).toBeGreaterThanOrEqual(9);
});
