...
The result: over 884 decided rounds in thirteen sessions, downstacking is the only printed
measure that becomes MORE decisive as rounds intensify — paired AUC across terciles of
combined VS runs 64.8 → 69.2 → 76.8 for the per-piece rate (Spearman rho +0.157 against
intensity; raw 清走 +0.148). It survives both controls the closing note quotes: it is not
round LENGTH (the same test against duration is rho +0.034, while APM's and 攻擊's apparent
decay IS a length effect at rho −0.173 / −0.174 — which is why the attacking lede this
...
rho +0.249, and 食 — which carries that same death bias but no skill — does not trend,
rho +0.092).

**The ninth session moved every one of those figures and moved none of the conclusions;
the tenth through thirteenth did it again.** The tercile progression flattened at both ends
...
63.1 → 68.6 → 77.8, and 09-19 raised the bottom two while lowering the top to
64.8 → 69.2 → 76.8 — the first movement that COMPRESSES the progression from both ends at once;
the headline rho went
+0.203 → +0.177 → +0.183 → +0.182 → +0.178 → +0.157, i.e. down, back up, barely at all, down
again, and then the largest single fall the series has had (0.021, against a previous largest of
0.026 at 09-03 and nothing else above 0.006).
...
Five sessions of movement — down, up, barely, down, down hard — is the clearest available
...
The 884 rounds are nested in matches, in sessions and in two players, so every p here
...
CORPUS = {
    ...
    "terciles": ["64.8", "69.2", "76.8"],
    "tests": {
        "cleared_pp/intensity": {"rho": "+0.157", "raw": "0.0001", "adj": "0.0001"},
        "cleared/intensity": {"rho": "+0.148", "raw": "0.0001", "adj": "0.0003"},
        "cleared_pp/duration": {"rho": "+0.034", "raw": "0.3076", "adj": "1.0000"},
        "apm/duration": {"rho": "-0.173", "raw": "0.0001", "adj": "0.0001"},
        "attack/duration": {"rho": "-0.174", "raw": "0.0001", "adj": "0.0001"},
        "cleared_per_received/intensity": {"rho": "+0.249", "raw": "0.0001", "adj": "0.0001"},
        "received/intensity": {"rho": "+0.092", "raw": "0.0064", "adj": "0.1661"},
    },
}
