...
The result: over 734 decided rounds in twelve sessions, downstacking is the only printed
measure that becomes MORE decisive as rounds intensify — paired AUC across terciles of
combined VS runs 63.1 → 68.6 → 77.8 for the per-piece rate (Spearman rho +0.178 against
intensity; raw 清走 +0.168). It survives both controls the closing note quotes: it is not
round LENGTH (the same test against duration is rho +0.053, while APM's and 攻擊's apparent
decay IS a length effect at rho −0.187 / −0.187 — which is why the attacking lede this
...
rho +0.244, and 食 — which carries that same death bias but no skill — does not trend,
rho +0.087).

**The ninth session moved every one of those figures and moved none of the conclusions;
the tenth, eleventh and twelfth did it again.** The tercile progression flattened at both ends
over 09-03 (62.6 → 67.0 → 80.9 became 62.2 → 68.7 → 77.1), 09-10 moved the middle to
62.1 → 67.5 → 77.1, 09-11 lifted both ends to 62.3 → 67.5 → 77.5, and 09-17 lifted all three to
63.1 → 68.6 → 77.8; the headline rho went
+0.203 → +0.177 → +0.183 → +0.182 → +0.178, i.e. down, back up, barely at all, and down again.
...
Four sessions of movement — down, up, barely, down — is the clearest available demonstration
that the digits are not the finding: a reader who could predict the next one from the last two
would have been wrong every time, and the fourth movement is the one that makes 「not moving at
the third decimal」 read as the coincidence it was rather than as convergence.
...
CORPUS = {
    ...
    "terciles": ["63.1", "68.6", "77.8"],
    "tests": {
        "cleared_pp/intensity": {"rho": "+0.178", "raw": "0.0001", "adj": "0.0001"},
        "cleared/intensity": {"rho": "+0.168", "raw": "0.0001", "adj": "0.0002"},
        "cleared_pp/duration": {"rho": "+0.053", "raw": "0.1515", "adj": "1.0000"},
        "apm/duration": {"rho": "-0.187", "raw": "0.0001", "adj": "0.0001"},
        "attack/duration": {"rho": "-0.187", "raw": "0.0001", "adj": "0.0001"},
        "cleared_per_received/intensity": {"rho": "+0.244", "raw": "0.0001", "adj": "0.0001"},
        "received/intensity": {"rho": "+0.087", "raw": "0.0189", "adj": "0.4899"},
    },
}
