...
- **97.9% here is not the 96.0% in the dual-backed section below**, and the two must not be quoted
  ...
  every session on disk by `cross-extract.mjs --check` and is at eleven. Comparing them now compares
...
over the verified prefix, sim and Triangle agree bit-exact on **96.0%** of locks, backing **96.1%** of
forecast events and **94.7%** of opener rounds with an independent engine — the dual-implementation
