...
OUT_PATH = os.path.join(os.path.dirname(__file__), "facts-24.json")
...
def file_index(fname):
    ...
    m = re.match(r"replay-2026-07-24-(\d+)\.ttrm$", fname)
    if not m:
...
def main():
    files = [f for f in os.listdir(PARENT_DIR)
             if re.match(r"replay-2026-07-24-\d+\.ttrm$", f)]
    if len(files) != 7:
        warn(f"Expected 7 .ttrm files, found {len(files)}: {sorted(files)}")
