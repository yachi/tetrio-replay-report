...
function fileIndex(filename: string): number {
  // replay-2026-07-22-.ttrm -> 1 ; replay-2026-07-22-2.ttrm -> 2 ; ... -10.ttrm -> 10
  const m = filename.match(/^replay-2026-07-22-(\d*)\.ttrm$/);
...
function main() {
  ...
  if (files.length !== 10) {
    warn(`Expected 10 .ttrm files, found ${files.length}`);
  }
