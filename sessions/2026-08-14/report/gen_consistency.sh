...
#
# Like every session from 07-28 on, this one has no bespoke codegen_dafny.py: its hand
# claims carry specs, so `pipeline.codegen` renders them alongside the generated ones
# into a single Facts.dfy / Claims.dfy. The ledger order here must match the one
# `pipeline.codegen.session_ledgers` produces, or the byte comparison fails for a reason
