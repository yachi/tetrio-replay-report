...
#
# Like 07-28 and unlike the two sessions before it, this session has no bespoke
# codegen_dafny.py: its hand claims carry specs, so `pipeline.codegen` renders them
# alongside the generated ones into a single Facts.dfy / Claims.dfy. The ledger order
# here must match the one `pipeline.codegen.session_ledgers` produces, or the byte
# comparison fails for a reason that has nothing to do with the data.
set -eu
cd "$(dirname "$0")"
REPO="$(cd ../../.. && pwd)"
